...
use eth_types::{BlockSelector, EngineTypes, HexBytes, TxTrait, SH256};
...
impl TransactionArg {
    pub fn to_legacy<E: EngineTypes>(
        &self,
        ...
        let mut tx = <E::Transaction>::legacy(
            nonce.as_u64().into(),
            gas_price,
            gas_limit.as_u64().into(),
            self.to.into(),
            self.value,
            self.data.clone(),
        );
    ...
    E: EngineTypes,
{
    ...
    mempool: Arc<Mutex<BTreeMap<SH160, BTreeMap<usize, MempoolTx<E>>>>>,
    seq: Arc<AtomicUsize>,
    resend_timeout: Duration,
    started: Arc<AtomicBool>,
}

impl<C, E> TxSender<C, E>
where
    C: RpcClient + Clone + Send + 'static,
    E: EngineTypes,
{
    pub fn new(alive: &Alive, chain_id: u64, el: ExecutionClient<C, E>, resend_timeout: Duration) -> Self {
        let sender = Self {
            alive: alive.clone(),
            chain_id,
            el,
    ...
    fn estimate_gas(&self, sender: &SH160, tx: &TransactionArg) -> Result<SU256, RpcError> {
        ...
        let mut tx_map = tx.to_json_map();
        tx_map.insert("from".into(), format!("{:?}", sender).into());

        let gas = self.el.estimate_gas(&tx_map, BlockSelector::Latest)?;
        Ok(gas)
    }
    ...
    pub fn send(
        ...
        let hash = self.el.send_raw_transaction(&tx.to_legacy::<E>(
            &sender,
            nonce.nonce,
            gas_price,
            gas_limit.clone(),
    ...
    fn remove_tx_notify(&self, acc: &SH160, seq: usize, status: TxReceiptStatus<E>) {
        if let Some(tx) = self.remove_tx(acc, seq) {
            let _ = tx.sender.send(status);
        }
    }

    fn remove_tx(&self, acc: &SH160, seq: usize) -> Option<MempoolTx<E>> {
        let mut mempool = self.mempool.lock().unwrap();
        let sender_txs = mempool.get_mut(acc)?;
        sender_txs.remove(&seq)
    }

    fn peek_tx(&self, acc: &SH160, start_seq: usize) -> Option<(usize, MempoolTx<E>)> {
    ...
    fn get_any_receipt(&self, hashes: &[SH256]) -> Result<Option<E::Receipt>, RpcError> {
        for hash in hashes {
                    ...
                    match tx.status {
                        MempoolTxStatus::Sent((hash, _)) => {
                            ...
                            match result {
                                ...
                                Ok(None) => {
                                    if Time::now() - tx.send_time > self.resend_timeout {
                                        ...
                                        let tx_inner = tx.tx.to_legacy::<E>(
                                            &sender,
                                            tx.nonce,
                                            gas_price,
                                            gas_limit.clone(),
...
#[derive(Clone, Debug)]
struct MempoolTx<E: EngineTypes> {
    tx: Arc<TransactionArg>,
    nonce: SU256,
    sender: mpsc::SyncSender<TxReceiptStatus<E>>,
    status: MempoolTxStatus<E>,
    send_time: Time,
    gas_limit: SU256,
    last_check: Time,
    tx_hashes: Vec<SH256>,
...
#[derive(Clone, Debug)]
enum MempoolTxStatus<E: EngineTypes> {
    Sent((SH256, Option<String>)),
    Confirmed(E::Receipt),
}

#[derive(Debug)]
pub struct TxReceipt<E: EngineTypes> {
    pub addr: SH160,
    pub seq: usize,
    pub receiver: mpsc::Receiver<TxReceiptStatus<E>>,
    pub status: TxReceiptStatus<E>,
}

impl<E: EngineTypes> TxReceipt<E> {
    pub fn wait_finished<F>(&mut self, alive: &Alive, f: F) -> Option<E::Receipt>
    where
        F: Fn(&TxReceipt<E>),
    {
...
#[derive(Debug)]
pub enum TxReceiptStatus<E: EngineTypes> {
